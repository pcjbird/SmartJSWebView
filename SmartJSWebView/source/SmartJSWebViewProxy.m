...
-(void)webView:(UIWebView *)webView didCreateJavaScriptContext:(JSContext *)ctx
{
    ...
    NSString * deferredjs = [NSString stringWithContentsOfFile:[SDK_BUNDLE pathForResource:@"deferredjs-inject" ofType:@"js"] encoding:NSUTF8StringEncoding error:nil];
    NSString * smartjs = [NSString stringWithContentsOfFile:[SDK_BUNDLE pathForResource:@"smartjs-inject" ofType:@"js"] encoding:NSUTF8StringEncoding error:nil];
    [webView stringByEvaluatingJavaScriptFromString:functionjs];
    [webView stringByEvaluatingJavaScriptFromString:deferredjs];
    [webView stringByEvaluatingJavaScriptFromString:smartjs];
    ...
    if ([requestString hasPrefix:@"smart-js:"]) {
        ...
        if ([sig methodReturnLength] > 0){
            ...
            if(!strcmp(returnType, @encode(id)))
            {
                ...
                {
                    [webView stringByEvaluatingJavaScriptFromString:@"SmartJS.retValue=null;"];
                }
                else
                {
                    if ([retValue isKindOfClass:[NSString class]])
                    {
                        ...
                        [webView stringByEvaluatingJavaScriptFromString:[@"" stringByAppendingFormat:@"SmartJS.retValue=\"%@\";", retValue]];
                    }
                    else
                    {
                        [webView stringByEvaluatingJavaScriptFromString:[@"" stringByAppendingFormat:@"SmartJS.retValue=\"%@\";", retValue]];
            ...
            if(!strcmp(returnType, @encode(BOOL)))
            {
                ...
                [webView stringByEvaluatingJavaScriptFromString:[@"" stringByAppendingFormat:@"SmartJS.retValue=%@;", result ? @"true" : @"false"]];
            }
            else if(!strcmp(returnType, @encode(int)))
            {
                int result = *((int*)_retValue);
                [webView stringByEvaluatingJavaScriptFromString:[@"" stringByAppendingFormat:@"SmartJS.retValue=%d;", result]];
            }
            else if(!strcmp(returnType, @encode(float)))
            {
                float result = *((float*)_retValue);
                [webView stringByEvaluatingJavaScriptFromString:[@"" stringByAppendingFormat:@"SmartJS.retValue=%f;", result]];
            }
            else
            {
